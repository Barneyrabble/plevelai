...
constexpr float PAN_STEPS_PER_DEG = 6400.0f / 360.0f;
constexpr float TILT_STEPS_PER_DEG = 6400.0f / 360.0f;
...
// Laser TTL configuration (defaults can be overridden at runtime via `config`).
constexpr uint8_t LASER_TTL_PIN = 10;
constexpr bool LASER_DEFAULT_ACTIVE_LOW = false;
constexpr float LASER_DEFAULT_MIN_CONFIDENCE = 0.0f;
...
struct CommandQueue {
  ...
  bool push(const TargetCommand &cmd) {
...
struct LaserConfig {
  ...
  float minConfidence = LASER_DEFAULT_MIN_CONFIDENCE;
};
...
void resetLaserConfig() {
  ...
  g_laserConfig.minConfidence = LASER_DEFAULT_MIN_CONFIDENCE;
}

uint8_t laserActiveLevel() {
  return g_laserConfig.activeLow ? LOW : HIGH;
}

uint8_t laserInactiveLevel() {
  return g_laserConfig.activeLow ? HIGH : LOW;
}

void laserWriteIdleLevel() {
  digitalWrite(LASER_TTL_PIN, laserInactiveLevel());
}
...
bool stepIfDue(Axis &axis, int direction, unsigned long nowMicros, unsigned long interval)
{
...
{
  unsigned long moveInterval = intervalFor(axis.moveStepsPerSec);
  unsigned long homeInterval = intervalFor(axis.homeStepsPerSec);
